...
class TestGame(unittest.TestCase):
    ...
    def test_get_straight_high_regular(self):
        ...
        self.assertEqual(self.game.getStraightHigh(hand_vals), 14)

    def test_get_straight_high_ace_low(self):
        hand_vals = [14, 5, 4, 3, 2, 9, 8]
        self.assertEqual(self.game.getStraightHigh(hand_vals), 5)

    def test_get_straight_high_none(self):
        hand_vals = [14, 13, 11, 9, 7, 4, 2]
        self.assertEqual(self.game.getStraightHigh(hand_vals), 0)

    def test_high_card(self):
        cards = make_cards(["AS", "KD", "9H", "7C", "4S", "3D", "2H"])
        score = self.game.evaluateHand(cards)
    ...
    def test_higher_pair_beats_lower_pair(self):
        ...
        score1 = self.game.evaluateHand(cards1)
        score2 = self.game.evaluateHand(cards2)
    ...
    def test_straight_flush_beats_four_of_a_kind(self):
        ...
        score1 = self.game.evaluateHand(straight_flush)
        score2 = self.game.evaluateHand(quads)
        self.assertGreater(score1, score2)

    def test_format_hand_high_card(self):
        score = (0, 14, 13, 9, 7, 4)
        self.assertEqual(self.game.formatHand(score), "High card: A K 9 7 4")

    def test_format_hand_pair(self):
        score = (1, 14, 9, 7, 4)
        self.assertEqual(self.game.formatHand(score), "Pair: As (9 7 4 kickers)")

    def test_format_hand_straight(self):
        score = (4, 14)
        self.assertEqual(self.game.formatHand(score), "Straight: A K Q J 10")

    def test_format_hand_ace_low_straight(self):
        score = (4, 5)
        self.assertEqual(self.game.formatHand(score), "Straight: 5 4 3 2 A")

    def test_format_hand_full_house(self):
        score = (6, 14, 13)
        self.assertEqual(self.game.formatHand(score), "Full House: As full of Ks")
